      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="font-medium text-white truncate">{reminder.lead.name}</h4>
            {isOverdue && (
              <span className="flex items-center gap-1 px-2 py-0.5 bg-red-900/50 text-red-400 text-xs rounded-full">
          ...
          {reminder.note && (
            <p className="text-sm text-gray-400 mt-1">{reminder.note}</p>
          )}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isPending}
            className="flex items-center gap-1 px-3 py-1.5 bg-green-900/30 text-green-400 rounded-lg hover:bg-green-900/50 text-sm"
            title="Mark complete"
          >
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-1 py-1 bg-gray-800 border border-gray-600 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
              <button
                onClick={() => handleSnooze(1)}
                disabled={isPending}
                className="block w-full px-4 py-2 text-left text-sm text-gray-200 hover:bg-gray-700 whitespace-nowrap"
              >
                Tomorrow
              </button>
              <button
  ...
  if (totalCount === 0) {
    return (
      <div className="text-center py-12 bg-gray-800 rounded-xl">
        ...
        <h3 className="text-lg font-medium text-gray-300 mb-2">No reminders</h3>
